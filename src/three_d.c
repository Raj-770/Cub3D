...
/*   Updated: 2024/06/01 15:27:41 by fnikzad          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// 		{
// 			int tex_x = (int)(pixelx) % tex->width;
// 			int tex_y = (int)(pixely) % tex->height;
// 			uint8_t *pixel = tex->pixels + (tex_y * tex->width * tex->bytes_per_pixel) + (tex_x * tex->bytes_per_pixel);
// 			color = *(int *) pixel;

// 			// Convert color from RGBA to the format used by mlx_put_pixel if necessary
...
int draw_line_3d(t_cub *game, t_v_line line, mlx_texture_t *tex) {
    ...
    while (pixels > 0) {
        if (pixelx >= 0 && pixelx < WIDTH && pixely >= 0 && pixely < HEIGHT) {
            int tex_x = (int)((pixelx - line.begin_x) * tex->width / (line.end_x - line.begin_x));
            int tex_y = (int)texture_pos;

            // Ensure texture coordinates are wrapped correctly
            tex_x = tex_x % tex->width;
            tex_y = tex_y % tex->height;

            // Fetch the pixel color from the texture
            if (tex_x >= 0 && tex_x < (int)tex->width && tex_y >= 0 && tex_y < (int)tex->height) {
                uint8_t *pixel = tex->pixels + (tex_y * tex->width + tex_x) * tex->bytes_per_pixel;
                uint32_t red = pixel[0];
                uint32_t green = pixel[1];
                uint32_t blue = pixel[2];
                uint32_t alpha = pixel[3];
